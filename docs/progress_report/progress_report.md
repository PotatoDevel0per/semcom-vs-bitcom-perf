...
This progress report summarizes work completed to date on a performance comparison between semantic communication and raw-bit (JPEG-compressed) transmission for task-oriented mobile computing. We have established a full simulation pipeline using CIFAR-10 image classification as the downstream task. A ResNet-18 classifier serving as the baseline achieved 93.51% test accuracy. Semantic encoders with latent dimensions of 64, 128, and 256 were trained end-to-end and evaluated under AWGN and packet-loss channel conditions. Initial results show that the semantic approach (dim=64, 64 bytes) achieves 91.94% accuracy at SNR=10 dB — using only 2.1% of the bandwidth required by the lossless baseline — while outperforming JPEG compression by a factor of approximately 15× in Accuracy per Bit.

---

## 1. Introduction

The proliferation of AI-driven mobile applications demands efficient transmission of data for inference tasks over bandwidth-constrained wireless channels. Conventional raw-bit transmission, even with source coding such as JPEG, transmits pixel-level information regardless of the downstream task, leading to poor utilization of limited channel resources. Semantic communication addresses this by extracting and transmitting only task-relevant features, thereby reducing the payload while preserving task performance [1][2].
...
Given a bandwidth-constrained wireless channel, the transmitter must send a representation of an image $x$ such that the receiver can correctly perform image classification. The key tension is between transmission size (bytes) and task accuracy. Formally, for a transmission budget $B$ (bytes), we compare:
...
The channel is modeled as AWGN with varying SNR, Rayleigh fading, or block-level packet loss.
...
The full experimental pipeline has been implemented in Python/PyTorch and is version-controlled on GitHub. The system runs on Apple MPS (M-series GPU). Key components:
...
A ResNet-18 classifier was adapted for CIFAR-10 (3×3 first conv, stride 1, no max-pooling) and trained for 30 epochs using SGD with cosine annealing (lr=0.1, weight decay=5×10⁻⁴, momentum=0.9). The model achieved **93.51% test accuracy**, which serves as the upper-bound reference for lossless transmission.
...
JPEG compression was evaluated at nine quality levels. The classifier (ResNet-18, §3.2) was applied to the decompressed images without retraining.
...
**(3) AWGN robustness:** Even at SNR=0 dB, semantic dim=256 maintains 91.43% accuracy. The dim=64 model shows greater sensitivity to low SNR (88.67% at 0 dB) due to its compressed representation.

**(4) Packet loss robustness:** All semantic models are remarkably robust to packet loss. Dim=256 accuracy fluctuates by less than 0.05% across 1%–10% loss rates, suggesting that the latent representation distributes task-relevant information redundantly across dimensions.
...
**Changes from original plan:** No major deviations. Rayleigh fading full evaluation is deferred to the next phase (April 9–20) to prioritize Progress Report submission.
